...
> **Documento autogenerado.** Derivado del commit: consultar `git log -1 --format=%H -- .cursor/`
>
> ⚠️ **REGLA OBLIGATORIA:** Todo cambio en archivos fuente DEBE ir acompañado de la actualización de los documentos agénticos correspondientes en `.cursor/`. Usa `doc-mapping.json` para identificar qué documentos actualizar según los archivos modificados. Ejecuta `python scripts/validate_docs.py --strict` antes de hacer commit.
>
> 📚 **PUNTO DE ENTRADA ÚNICO:** Lee primero `.cursor/README.md` para conocer la estructura completa de la documentación agéntica. `.cursor/` es la única fuente de verdad.
...
| **Modificar documentación agéntica** | Editar `.cursor/agent/*.md`, `.cursor/context/*.md`, `.cursor/learning/*.md` |

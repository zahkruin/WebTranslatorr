...
|---------|--------|-------------|----------|
| `.cursor/AGENTS.md` | ✅ Vigente | Alta | Actualizado — visión general, stack, convenciones, anti-patrones |
| `.cursor/INDEX.md` | ✅ Vigente | Alta | Mapa completo de casos de uso → documentación |
| `.cursor/styleguide.md` | ✅ Vigente | Media | Convenciones de código (podría detallar más patrones) |
| `.cursor/doc-mapping.json` | ✅ Vigente | Alta | 30 reglas de mapeo — cobertura completa de archivos fuente |
| `.cursor/phase0-ficha-tecnica.yaml` | ✅ Vigente | Alta | Ficha técnica precisa |
| `.cursor/phase1-modules.json` | ✅ Vigente | Alta | 24 módulos clasificados con dependencias |
| `.cursor/phase2-dynamic.json` | ✅ Vigente | Alta | Análisis dinámico con issues detectados |

### Documentos de Contexto (.cursor/context/)
...
|---------|--------|-------------|
| `README.md` | ✅ Vigente | Media — funcional pero breve |
| `docs/architecture.md` | 🗂️ Centralizado | Migrado a `.cursor/legacy/docs-architecture.md` (redundante con 01-architecture.md) |
| `docs/adding_providers.md` | 🗂️ Centralizado | Migrado a `.cursor/legacy/docs-adding-providers.md` (redundante con 17-adding-providers.md) |
| `implementation_plan.md` | 🗂️ Centralizado | Migrado a `.cursor/legacy/implementation_plan.md` |
| `integration_plan.md` | 🗂️ Centralizado | Migrado a `.cursor/legacy/integration_plan.md` |
| `plans/*.md` (7 archivos) | 🗂️ Centralizado | Migrados a `.cursor/plans/` |

### Documentación Paralela (.gemini/)

| Contenido | Estado |
|-----------|--------|
| `.gemini/` (copia de .cursor/) | ✅ Eliminado | Contenido único fusionado en `.cursor/context/`, el resto era duplicado |

> **Nota (2026-08-17):** centralización completada. `.gemini/`, `.plans/`, `plans/`, `docs/`, `implementation_plan.md` e `integration_plan.md` han sido consolidados en `.cursor/`. Ver `.cursor/README.md`.
...
| Documentación duplicada en `docs/` y `.gemini/` | ✅ Resuelto | Centralizada en `.cursor/` (2026-08-17) |
...
5. **Prioridad 5**: ~~Deprecar `docs/` y `.gemini/` en favor de `.cursor/`~~ ✅ Completado (2026-08-17).

...
def patched_app(memory_connection):
    ...
    # Patch get_db wherever the admin endpoints might import it.
    # The admin module imports get_db *inside* function bodies, so we patch
    # the source location.
    with patch(
        "app.persistence.database.get_db", return_value=memory_connection
    ):
        # Also patch inside app.services.config_manager
        with patch(
            "app.services.config_manager.get_db", return_value=memory_connection
        ):
            app.state.config_manager = ConfigManager()

    # Mock domain_resolver for the reload endpoint
...
class TestProviderEndpoints:
    ...
    async def test_reload_providers(self, client):
        ...
        with patch("app.api.admin.torznab") as mock_torznab:
            mock_torznab._init_providers = AsyncMock()

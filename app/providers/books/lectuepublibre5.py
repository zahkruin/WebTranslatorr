"""
Provider para lectuepublibre5.com - Libros electrónicos en español.
...
logger = logging.getLogger("provider.lectuepublibre5")


class LectuEpubLibre5Provider(BaseProvider):
    ...
    async def search(
        ...
        **kwargs
        ...
        self.logger.info(f"Buscando en LectuEpubLibre5: '{query_to_use}'")
        if not query_to_use:
            return []
        ...
        if self._USE_API:
            try:
                ...
                if api_results:
                    self.logger.info(
                        f"LectuEpubLibre5 API: {len(api_results)} results for '{query_to_use}'"
                    )
                    return api_results
            except Exception as e:
                self.logger.warning(f"LectuEpubLibre5 API fallback: {e}")

        # ── Strategy 2: HTML scraping (fallback) ──
        self.logger.info(f"LectuEpubLibre5: falling back to HTML scraping for '{query_to_use}'")
        ...
        try:
            ...
            for selector in ['a[href*="/book/"]', 'a[href*="/libro/"]',
                ...
                for a in soup.select(selector):
                    ...
                    result = SearchResult(
                        title=title_text,
                        guid=f"lectuepublibre5-{internal_id}",
            ...
            self.logger.error(f"Error parseando LectuEpubLibre5: {e}")
    ...
    async def browse(
        ...
        **kwargs
        ...
        self.logger.info(f"LectuEpubLibre5 browse: recent posts (offset={offset}, limit={limit})")
        try:
            return await self._api.list_recent(limit=limit, offset=offset)
        except Exception as e:
            self.logger.warning(f"LectuEpubLibre5 browse API error: {e}")
            return []

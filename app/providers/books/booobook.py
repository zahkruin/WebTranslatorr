...
class BooobookProvider(BaseProvider):
    ...
    async def browse(
        ...
        **kwargs
        ...
        self.logger.info(f"Booobook browse: homepage (offset={offset}, limit={limit})")
        results = []
        url = self.base_url
        ...
        try:
            ...
            seen_urls = set()
            for selector in ['a[href*="/book/"]', 'a[href*="/libro/"]', 
                             'h2.entry-title a', 'h3.entry-title a', 'article a[href*="/"]']:
                for a in soup.select(selector):
                    href = a.get('href')
                    title_text = a.get_text(strip=True)
                    if not href or not title_text or href in seen_urls:
                        continue
                    if len(title_text) < 3 or any(w in title_text.lower() for w in ('home', 'inicio', 'blog')):
                        continue
                    seen_urls.add(href)
                    match = re_mod.search(r'/(?:book|libro)/([^/]+)/?', href)
                    internal_id = match.group(1) if match else (
                        [p for p in href.rstrip('/').split('/') if p][-1] if '/' in href else None
                    )
                    if not internal_id:
                        continue
                    result = SearchResult(
            ...
            self.logger.error(f"Booobook browse error: {e}")

...
class HolaEbookProvider(BaseProvider):
    ...
    async def browse(
        ...
        **kwargs
        ...
        self.logger.info(f"HolaEbook browse: homepage (offset={offset}, limit={limit})")
        results = []
        url = self.base_url
        ...
        try:
            ...
            for a in soup.select('a[href*="/libro"], a[href*="/book"]'):
                ...
                if not href or not title_text or href in seen_urls:
                    continue
                seen_urls.add(href)
                parts = [p for p in href.rstrip('/').split('/') if p and p not in ('libro', 'book')]
                internal_id = parts[-1].replace('.html', '') if parts else None
                if not internal_id:
                    continue
                result = SearchResult(
                    title=title_text,
            ...
            self.logger.error(f"HolaEbook browse error: {e}")

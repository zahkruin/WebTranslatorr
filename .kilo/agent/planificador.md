...
6. `.kilo/context/` — TODOS los documentos de contexto aplicables al proyecto
7. `.kilo/doc-mapping.json` — Mapeo archivos fuente → documentación
...
11. `.kilo/phase1-modules.json` — Estructura de módulos
12. `.kilo/phase2-dynamic.json` — Issues detectados previamente

## Ámbito de actuación
- **Puede modificar**: Exclusivamente documentos de plan en `.kilo/plans/`. No modifica ningún archivo de código fuente.
- **No puede modificar**: `app/`, `tests/`, `config.py`, `main.py`, `Dockerfile`, `docker-compose.yml`, `requirements.txt`, documentación de contexto (`.kilo/context/`), definiciones de agentes, CHANGELOG.md
...
- Entregar el plan al orquestador en formato `.kilo/plans/{id}-plan.md`
...
3. CONSULTAR ANTECEDENTES
   ...
   → Leer .kilo/learning/planificador/experiences.md (planes previos similares)
   → Leer .kilo/learning/planificador/errors.md (errores de planificación previos)
...
6. ENTREGAR AL ORQUESTADOR
   → Guardar el plan en .kilo/plans/{id}-plan.md
...
## 10. Referencias
- **Documentación relacionada**: `.kilo/context/...`, `.kilo/analysis/...`
- **Issues/PRs**: #ID
